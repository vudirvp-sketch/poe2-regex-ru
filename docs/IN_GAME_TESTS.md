...
11. **`AND(LITERAL_ctx, LITERAL_regex)` без EXCLUDE inside OR → `ctx.*regex` (iter 108)** — `regexPrefixContext` без `regexExclude` компилируется в same-block AND через `.*` bridge (NOT cross-block AND `"ctx" "regex"`). До iter 108 cross-block AND внутри внешнего OR-quote давал **вложенные кавычки** → PoE2 zero matches (rule B0). iter 108 фикс в `src/core/compiler.ts` `normalizeAst` OR-case.
    **Post-fix аудит (exhaustive, 4 сценария × 10 категорий):** 0 критических нарушений (B0/NESTED/EMPTY) по всем 1697 токенам + 543 opt-table entries. Аудит-скрипт: `/home/z/my-project/scripts/audit-corrected.ts`. LIMIT-overflow (>250 chars) — 20 случаев, обрабатываются runtime split (`splitOverLimitRegex`, iter 50). Regression tests в `tests/core/optimizer.test.ts` (`iter 108: AND(LITERAL...) WITHOUT EXCLUDE inside OR` section, 10 тестов включая full 7-affix Abyss tablet scenario).

---

## iter 108 Exhaustive Audit — Global Compliance Check

**Цель:** проверить, что фикс iter 108 globally effective — нет аналогичных багов в других категориях.

**Аудит-скрипт:** `/home/z/my-project/scripts/audit-corrected.ts`

**4 тестовых сценария на каждую категорию:**

| Test | Scenario | Цель |
|------|----------|------|
| T1 | Single-token в 2-element OR (минимальный case для iter 108 transform) | Базовая корректность каждого токена |
| T2 | All-tokens в OR (worst-case, вся категория) | Полная OR-компиляция |
| T3 | opt-table pre-compiled regexes | ETL-генерированные регексы в JSON |
| T4 | Family-AND (top-level AND mode, OR внутри family) | Cross-family AND mode |

**Правила проверки:**

| Rule | Pattern | Что означает |
|------|---------|--------------|
| B0 / NESTED | regex содержит `\|"` (pipe рядом с quote) | iter 108 bug signature — zero matches |
| EMPTY_REGEX | regex = `""` или пустой | Токен без regex.ru |
| LIMIT | regex.length > 250 | Превышение PoE2 лимита (handled at runtime) |

**Результаты (10 категорий, 1697 токенов, 543 opt-table entries):**

| Метрика | Значение |
|---------|----------|
| B0 / NESTED violations | **0** |
| EMPTY_REGEX | **0** |
| LIMIT (>250 chars) | 20 (inherent, runtime-split handled) |
| T1 single-token OR violations | **0** |

**Вывод:** iter 108 фикс **глобально эффективен** — 0 критических нарушений во всех категориях. LIMIT-overflow не является багом и обрабатывается runtime-сplit-логикой (iter 50, `splitOverLimitRegex` в `src/core/limits.ts`).
