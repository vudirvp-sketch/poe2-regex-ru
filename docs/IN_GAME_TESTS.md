...
> **Верификация v6:** 2026-06-16 — тесты 15-17: `|` с многословными сломан на всех уровнях.
...
## Iteration 36 — OR с многословными паттернами: окончательные тесты

### T15. `|` внутри `()` с многословными — ❌ НЕ РАБОТАЕТ

**Regex:** `(скорости атаки|передвижения)`

**Результат:** Ничего не подсвечено. У тестировщика нет самоцветов на скорость передвижения, но даже самоцветы со «скорости атаки» не были подсвечены.

**Вывод:** `()` НЕ сохраняет `|` для многословных альтернатив. `|` внутри `()` с пробелами — сломан.

### T16. `|` внутри `()` внутри кавычек — ❌ НЕ РАБОТАЕТ (мусор)

**Regex:** `"повышение (брони|скорости)"`

**Результат:** Подсветило ВСЁ, где есть слово «повышение» (и, возможно, «скорости»). Регекс слишком широкий — `(брони|скорости)` внутри `"..."` не фильтрует, совпадает только общий префикс «повышение».

**Вывод:** `()` с `|` внутри `"..."` не работает как группировка OR для многословных. Поисковый движок игнорирует или неправильно парсит `(брони|скорости)`, совпадает только «повышение».

### T17. Полный opt-паттерн — ❌ НЕ РАБОТАЕТ (мусор)

**Regex:** `"повышение (брони|скорости атаки|шанса критического удара)"`

**Результат:** Подсветило много мусора — аналогично Т16. Совпадает «повышение» + возможно «скорости», но без точной фильтрации альтернатив.

**Вывод:** Оптимизационная таблица, генерирующая паттерны вида `"префикс (альтернатива1|альтернатива2|альтернатива3)"`, **фундаментально неработоспособна** для многословных альтернатив.

### Итог тестов 15-17

`|` между многословными альтернативами **сломан на всех уровнях** — верхний уровень, внутри `()`, внутри `"..."` + `()`. Оптимизационная таблица сломана для ~90% записей (106/113 в самоцветах). **Путь B (полный редизайн) подтверждён.**
