...
## Positive + Negative Pattern — NOT YET VERIFIED IN-GAME

Формат `"want" !"dontwant"` теоретически поддерживается PoE2 (диалект поддерживает `!` и `|`),
но **не проверен в игре** для паттерна с одновременным want + exclude.

| # | Regex | Ожидание | Статус |
|---|-------|----------|--------|
| PN1 | `"к сопротивлению огню" !"к сопротивлению холоду"` | Подсветить предметы с огнём, но БЕЗ холодом | ⏳ НЕ ПРОВЕРЕН |
| PN2 | `"к максимуму здоровья" !"к сопротивлению"` | Здоровье есть, сопротивлений нет | ⏳ НЕ ПРОВЕРЕН |

**Заметки:**
- `!` negation — item-wide (проверяет ВСЕ блоки предмета)
- `!` должно быть ВНУТРИ кавычек при комбинации с `|`: `"!A|B"` работает, `!"A|B"` НЕТ
- UI теперь поддерживает per-mod exclude toggle (✗/✓ кнопка на каждом чипе)
