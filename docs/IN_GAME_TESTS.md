...
## Positive + Negative Pattern — VERIFIED IN-GAME (2026-06-10)

Формат `"want" "!dontwant"` — **подтверждён в игре**. Компилятор генерирует `!` внутри кавычек,
что является единственным рабочим форматом в PoE2.

### In-game test results

| # | Regex | Ожидание | Результат |
|---|-------|----------|-----------|
| PN1 | `"к сопротивлению огню" "!к сопротивлению холоду"` | Подсветить предметы с огнём, но БЕЗ холодом | ✅ Работает |
| PN2 | `"к сопротивлению огню" !"к сопротивлению холоду"` | То же, но `!` вне кавычек | ❌ Ничего не подсвечивает |
| PN3 | `"к сопротивлению огню" "к сопротивлению холоду"` | Предметы с ОБ ОИ М сопротивлениями | ✅ Работает |

**Критическое правило:** `!` ДОЛЖНО быть внутри кавычек: `"!text"` работает, `!"text"` — НЕТ.
Это касается и `|`: `"!A|B"` работает (отрицание любого из A/B), `!"A|B"` — нет.

**Заметки:**
- `!` negation — item-wide (проверяет ВСЕ блоки предмета)
- UI поддерживает per-mod exclude toggle (✗/✓ кнопка на каждом чипе)
- Компилятор генерирует корректный формат автоматически
