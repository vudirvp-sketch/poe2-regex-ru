...
11. **`AND(LITERAL_ctx, LITERAL_regex)` без EXCLUDE inside OR → `ctx.*regex` (iter 108)** — `regexPrefixContext` без `regexExclude` компилируется в same-block AND через `.*` bridge (NOT cross-block AND `"ctx" "regex"`). До iter 108 cross-block AND внутри внешнего OR-quote давал **вложенные кавычки** → PoE2 zero matches (rule B0). iter 108 фикс в `src/core/compiler.ts` `normalizeAst` OR-case. Regression tests в `tests/core/optimizer.test.ts` (`iter 108: AND(LITERAL...) WITHOUT EXCLUDE inside OR` section, 10 тестов включая full 7-affix Abyss tablet scenario).
