# Atlas Jewel — План новой категории (iter 175 — РАЗВЕДКА)

> **Статус:** ПЛАНИРОВАНИЕ. Код НЕ изменялся. Ждёт согласования с пользователем.
> **Цель:** Добавить отдельный раздел для генерации регексов подсветки нод древа атласа, заменяемых особыми (Вневременными) самоцветами.
