...
6. **Per-mod numeric filter** — Current UI applies GLOBAL min/max to ALL ranged tokens. Need per-mod thresholds. See §9 below.
7. **SAPPHIRE generic crit rule conflict** — `/повышен.*шанс.*критического удара/` (w=2) conflicts with Emerald attack-crit. See §10 below.
...
| Vaal (Ваал) | `ваал` | ❌ |
| Expedition (Экспедиция) | `экспед` | ⛔ Not in game yet |
...
**Current:** Single GLOBAL `minValue`/`maxValue` applied to ALL ranged tokens.

**Desired:** Per-mod thresholds (e.g., waystone ≥80% monsters AND ≥96% experience).

**PoE2 regex supports this** via AND: `"([8-9].|\d..).*количество монстр" "([9-9].|\d..).*количество опыт"` — each quoted group has its own number+suffix constraint.

**Implementation (future):** Per-token `minValue`/`maxValue` in filter store, per-chip min/max inputs in UI, modify `buildAstFromSelections` to use per-token thresholds.

## 10. Scoring Conflict — SAPPHIRE Generic Crit

`SAPPHIRE_SCORES` `/повышен.*шанс.*критического удара/` (w=2) matches Emerald attack-crit mods. Resolution: narrow to `/повышен.*шанс.*критического удара(?!.*атак)/i` or remove (Sapphire has specific spell-crit rules already).
