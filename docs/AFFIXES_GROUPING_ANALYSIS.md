...
> **Статус:** Open Proposal OP-1 — iter 82 анализ → iter 83 верификация → iter 84 P0-фиксы → iter 85 infrastructure (7/24 блоков) → iter 86 +7 блоков (14/24) + production switch для ring/amulet/belt → iter 87 weapon sub-blocks для jewel (6 weapon-class sub-blocks) + production switch для jewel.
> **iter 87:** WeaponClass type + WEAPON_CLASS_LABELS + classifyWeaponClass + WEAPON_SPECIFIC_PATTERN + шаг 12 в classifyFunctionalBlock + `'jewel-functional'` mode в classifyGroups. **Production переключён** — JewelPage использует `jewel-functional`. jewel.json other-bucket = 21.8% (цель <30% достигнута). 1315/1315 tests.
...
|---|---|---|
| ring / belt / amulet | `affix-functional` (iter 86) | 14 функциональных блоков: Spirit / Skill levels / Attributes / Resources / Runes barrier / Resistances / Defence stats / Offence speed / Crit / Damage type / Minions / Flasks / MF / Breach / Other |
| jewel | `jewel-functional` (iter 87) | 14 функциональных блоков + `weapon-specific` раскрывается в 6 weapon-class sub-blocks (melee / bow / crossbow / staff / spear / dagger) |
...
## 5. Приоритезация (обновлено iter 87)
...
| **P1** | Снижение other-bucket jewel.json ниже 15% (добавить 2-3 блока: ailments / penetration / area-duration) | `src/shared/mod-classifier.ts` + `tests/shared/mod-classifier.test.ts` | не начато (iter 88) | jewel.json other-bucket 21.8% → <15% |
...
Реализованы 6 weapon-class sub-blocks (melee / bow / crossbow / staff / spear / dagger) для 24 weapon-specific family-keys в jewel.json. JewelPage переключён на новый режим `jewel-functional`.

**Что сделано:**
- Новый `WeaponClass` type (`melee | bow | crossbow | staff | spear | dagger`) + `WEAPON_CLASS_LABELS` (display config, 6 entries — каждая со своим цветом).
- `WEAPON_NAME_TO_CLASS` lookup table — 10 weapon name variants (мечами/кинжалами/топорами/булавами/луками/самострелами/копьями/боевыми посохами/кистенями/без оружия) → 6 weapon classes. «без оружия» strategically placed first в melee lookup (defensive — longest match first).
...
- В `classifyFunctionalBlock()` добавлен **шаг 12** (weapon-specific) ДО crit/damage-type/offence-speed — weapon mods имеют attack+damage/attack+speed/attack+critical теги, но функционально это weapon-conditional passives.
- Match priority обновлён с 15 до 16 шагов: spirit → runes-barrier → breach → magic-find → skill-levels → flasks → minions → attributes → resistances → resources → defence-stats → **weapon-specific** → crit → damage-type → offence-speed → other.
- Новый `ModGroupMode = 'jewel-functional'` — вариант `affix-functional`, где `weapon-specific` блок раскрывается в 6 sub-blocks (weapon-melee/bow/crossbow/staff/spear/dagger) + defensive `weapon-other` fallback.
- `classifyGroups(mode='jewel-functional')` — split logic с preserve-reference semantics.
- 47 новых тестов (1268 базовых + 47 = 1315 passing).
- Production switch: `groupMode="affix-semantic"` → `groupMode="jewel-functional"` в JewelPage.

**Симуляция (scripts/simulate-iter87-impact.ts) — 4 verification checks:**
...
**Распределение 24 family-keys по 6 weapon classes:**

| Weapon class | Count | Examples |
|---|---|---|
...
**Решение:** Production-страница JewelPage переключена на `jewel-functional` — 24 weapon family-keys организованы в 6 читаемых подблоков (вместо одной большой «offensive» корзины).
...
- `tests/shared/mod-classifier.test.ts` — +47 тестов (classifyWeaponClass для всех 6 классов + WEAPON_CLASS_LABELS + weapon-specific match priority + jewel-functional mode).
...
**Ключевые файлы, затронутые в iter 87:**
- `src/shared/mod-classifier.ts` (~1650 строк) — добавлены WeaponClass type + WEAPON_CLASS_LABELS + WEAPON_NAME_TO_CLASS + classifyWeaponClass + WEAPON_SPECIFIC_PATTERN + шаг 12 в `classifyFunctionalBlock()` + `'jewel-functional'` mode в `classifyGroups()`.
- `tests/shared/mod-classifier.test.ts` (~1725 строк) — +47 тестов (classifyWeaponClass для всех 6 классов + WEAPON_CLASS_LABELS + weapon-specific match priority + jewel-functional mode).
- `src/ui/pages/jewel/JewelPage.tsx` — `groupMode="jewel-functional"` + header docstring.
- `scripts/simulate-iter87-impact.ts` — новый скрипт для mirror-симуляции классификаторов на реальных jewel JSON.

**Ключевые файлы для будущих итераций (iter 88+):**
- `src/shared/mod-classifier.ts` — снизить other-bucket jewel.json ниже 15% (добавить 2-3 блока: ailments / penetration / area-duration).
