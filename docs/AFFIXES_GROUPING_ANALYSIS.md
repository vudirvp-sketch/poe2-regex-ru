...
> **Статус:** Open Proposal OP-1 — iter 82 анализ → iter 83 верификация → iter 84 P0-фиксы → iter 85 infrastructure (7/24 блоков).
> **iter 85:** FunctionalBlock type + FUNCTIONAL_BLOCK_LABELS (24 entries) + classifyFunctionalBlock (7 active patterns) + `affix-functional` mode в classifyGroups. **Production НЕ переключён** — other-bucket = 70.4% с 7 блоками (нужно ещё 5-7 блоков в iter 86, чтобы опуститься ниже 30% и переключить страницы).
...
## 5. Приоритезация (обновлено iter 86)
...
| **P0** ✅ | Реализованы 7 новых функциональных блоков (defence-stats / offence-speed / crit / damage-type / flasks / resources / minions) через tags + text patterns | `src/shared/mod-classifier.ts` | **done iter 86** | other-bucket снизился с 70.4% до 9.9% |
...
| **P0** ⏳ | Weapon sub-blocks для jewel (по 6 группам из §4.2) | `src/shared/mod-classifier.ts` + `ModList.tsx` | ждёт iter 87 | 24 family-key организованы в 6 читаемых подблоков |
| **P1** | ETL-tagged functionalCategory для ring/amulet/belt (по образцу jewelType) | `scripts/etl/normalize.ts` + `generate-dictionary.ts` + `fetch-poe2db.ts` | не начато | ~100% точность классификации |
...
Реализованы 7 новых функциональных блоков через **tags + text patterns** поверх 7 блоков iter 85. Production-страницы RingPage/AmuletPage/BeltPage **переключены на `affix-functional`**.

**Что сделано:**
- 7 новых паттернов (каждый с tags + text fallback):
  - `flasks` — text «флакон» (belt primary + amulet charm-tagged).
  - `minions` — tag `minion` + text «приспешник»/«подношен».
  - `resources` — tags `life`/`mana` + text «максимум.*энерг.*щит»/«похищен»/«регенерац»/«восстанавливает».
  - `defence-stats` — tags `armour`/`evasion`/`energy_shield`/`charm` + text «брон»/«уклонен»/«блок»/«порог оглушен».
  - `crit` — tag `critical` + text «крит».
  - `damage-type` — tags `damage`/`physical`/`elemental`/`cold`/`fire`/`lightning`/`chaos` + text «урон».
  - `offence-speed` — tag `speed` + text «скорость атаки/сотворения/передвижения/снарядов».
- `classifyFunctionalBlock()` переписана: сбор тегов из `group.members` (с пропуском Breach Lord source tags), 15-шаговый match priority с tags + text.
...
- Ключевые приоритеты тегов: `minion` бьёт `life`/`mana`/`critical`/`damage`/`speed`/`resistance`; `resistance` бьёт `fire`/`cold`/`lightning`/`chaos`/`elemental`; `life`/`mana` бьёт `energy_shield` через text «максимум.*энерг.*щит» (ES max → resources, ES recharge → defence-stats); `critical` бьёт `damage`.
- 52 новых теста (1216 базовых + 52 = 1268 passing).
- Production switch: `groupMode="affix-semantic"` → `groupMode="affix-functional"` в RingPage/AmuletPage/BeltPage.

**Симуляция (scripts/simulate-iter86-impact.ts):**

| Категория | Total groups | 14 блоков поймали | Other bucket |
|---|---|---|---|
| ring | 94 | 85 (90.4%) | 9 (9.6%) |
| amulet | 105 | 93 (88.6%) | 12 (11.4%) |
| belt | 85 | 78 (91.8%) | 7 (8.2%) |
| **Итого** | 284 | 256 (90.1%) | 28 (9.9%) |

**Решение:** Production-страницы переключены на `affix-functional` — other-bucket = 9.9% (цель <30% достигнута с запасом).

**Изменённые файлы (8):**
- `src/shared/mod-classifier.ts` — +7 новых паттернов + tag-based классификация в `classifyFunctionalBlock()`, обновлены комментарии.
- `tests/shared/mod-classifier.test.ts` — +52 теста (7 блоков + edge cases + match priority).
...
Реализована инфраструктура 24-блочной системы. **Production НЕ переключён.**

**Что сделано:**
- Новый тип `FunctionalBlock` (24 значения) + `FUNCTIONAL_BLOCK_LABELS` (display-конфиг для всех 24) + `FUNCTIONAL_BLOCK_ORDER` (порядок рендера — игрок-сценарий).
- `classifyFunctionalBlock(group): FunctionalBlock` — 7 активных паттернов:
  - `spirit` → `/к духу/i`
  - `skill-levels` → 7 альтернатив (уровень камней / качество / перезарядка умений / длительность эффекта умения)
  - `attributes` → 8 альтернатив (Сила/Ловкость/Интеллект/Все + dual-attr Breach Lord + снижение требований)
  - `resistances` → 2 альтернативы (сопротивление + добавленные свойства сопротивлений)
  - `runes-barrier` → `/руническ.*барьер/i`
  - `magic-find` → 2 альтернативы (редкость / количество найденных предметов)
  - `breach` → `/Знак.*повелител.*Бездн/i`
- Match priority: spirit → runes-barrier → breach → magic-find → skill-levels → attributes → resistances → other (fallback).
- Новый режим `ModGroupMode = 'affix-functional'` добавлен в `classifyGroups()`.
...
**Решение:** НЕ переключать production-страницы на `affix-functional`, т.к. other-bucket = 70.4% — это регрессия относительно текущего neutral (17.7%). iter 86 добавит 5-7 блоков (defence-stats, offence-speed, crit, damage-type, flasks, resources, minions) — после этого other-bucket опустится достаточно для переключения.

**Изменённые файлы (5):**
- `src/shared/mod-classifier.ts` — FunctionalBlock type + FUNCTIONAL_BLOCK_LABELS + FUNCTIONAL_BLOCK_ORDER + 7 паттернов + classifyFunctionalBlock() + affix-functional mode.
- `tests/shared/mod-classifier.test.ts` — +44 теста.
...
**Ключевые файлы, затронутые в iter 86:**
- `src/shared/mod-classifier.ts` (~1530 строк) — добавлены 7 новых паттернов (FLASKS/MINIONS/RESOURCES/DEFENCE_STATS/CRIT/DAMAGE_TYPE/OFFENCE_SPEED) + tag-based классификация в `classifyFunctionalBlock()`.
- `tests/shared/mod-classifier.test.ts` (~1350 строк) — +52 теста (7 новых блоков + edge cases + match priority).
- `src/ui/pages/{ring,amulet,belt}/*.tsx` — `groupMode="affix-functional"`.
- `scripts/simulate-iter86-impact.ts` — новый скрипт для mirror-симуляции классификатора на реальных JSON.

**Ключевые файлы для будущих итераций (iter 87+):**
- `src/shared/mod-classifier.ts` — добавить weapon sub-blocks для jewel (6 подблоков для 24 family-key).
...
- `src/ui/components/VirtualizedModList.tsx` (811 строк) — поддержка weapon sub-blocks для jewel.
