...
## 5. Приоритезация (обновлено iter 85)
...
| **P0** ⏳ infrastructure | FunctionalBlock type + FUNCTIONAL_BLOCK_LABELS (24 entries) + classifyFunctionalBlock (7 active patterns) + `affix-functional` mode в classifyGroups + 44 теста | `src/shared/mod-classifier.ts` + `tests/shared/mod-classifier.test.ts` | **done iter 85** (infrastructure) | Готова к включению в iter 86 после добавления ещё 5-7 блоков |
| **P0** ⏳ next | Реализовать еще 5-7 функциональных блоков (defence-stats / offence-speed / crit / damage-type / flasks / resources / minions) для снижения other-bucket с 70% до <30% | `src/shared/mod-classifier.ts` | ждёт iter 86 | После этого — переключить ring/amulet/belt pages на `affix-functional` |
| **P0** ⏳ | Включить `affix-functional` groupMode в RingPage/AmuletPage/BeltPage | `src/ui/pages/{ring,amulet,belt}/*.tsx` | ждёт iter 86 | Снижение neutral с 17.7% до <5% (после всех 24 блоков) |
...
**iter 85:** Инфраструктура 24 функциональных блоков готова (7 активны). Production НЕ переключён — other-bucket = 70.4% с 7 блоками (нужно ещё 5-7 блоков в iter 86).

**Ключевые файлы, затронутые в iter 85:**
- `src/shared/mod-classifier.ts` (~1360 строк) — добавлен FunctionalBlock type + FUNCTIONAL_BLOCK_LABELS + FUNCTIONAL_BLOCK_ORDER + 7 паттернов + classifyFunctionalBlock() + affix-functional mode.
- `tests/shared/mod-classifier.test.ts` (~950 строк) — +44 теста (7 блоков + match priority + FUNCTIONAL_BLOCK_LABELS contract + classifyGroups integration).

**Ключевые файлы для будущих итераций:**
- `src/shared/mod-classifier.ts` — добавить ещё 5-7 блоков (defence-stats, offence-speed, crit, damage-type, flasks, resources, minions) в iter 86.
...
- `src/ui/components/VirtualizedModList.tsx` (811 строк) — переключение ring/amulet/belt pages на `affix-functional` mode в iter 86.
...
- `src/shared/i18n.ts` — новые метки 24 блоков (если потребуются вне FUNCTIONAL_BLOCK_LABELS).
