...
> **iter 88:** AILMENTS_PATTERN + AREA_DURATION_PATTERN (шаги 16/17 в classifyFunctionalBlock, после offence-speed, до other). jewel.json other-bucket 21.8% → **14.0%** (цель <15% достигнута). 1340/1340 tests.
...
|---|---|---|
| ring / belt / amulet | `affix-functional` (iter 86-88) | 17 функциональных блоков: Spirit / Skill levels / Attributes / Resources / Runes barrier / Resistances / Defence stats / Offence speed / Crit / Damage type / Ailments / Area-Duration / Minions / Flasks / Рарити / Breach / Other |
| jewel | `jewel-functional` (iter 87) | 17 функциональных блоков + `weapon-specific` раскрывается в 6 weapon-class sub-blocks (melee / bow / crossbow / staff / spear / dagger) |
...
Реализованы 2 новых функциональных блока поверх 15 блоков iter 87. jewel.json other-bucket 21.8% → **14.0%** (цель <15% достигнута). Заодно — UX-фикс: «Магический поиск» → «Рарити».
...
**Ключевые файлы, затронутые в iter 88:**
- `src/shared/mod-classifier.ts` (~1820 строк) — добавлены AILMENTS_PATTERN + AREA_DURATION_PATTERN + шаги 16/17 в `classifyFunctionalBlock()` + FUNCTIONAL_BLOCK_LABELS: «Магический поиск»→«Рарити», area-duration muted→violet.
- `tests/shared/mod-classifier.test.ts` (~1960 строк) — +25 тестов (ailments: 11 positive + 4 negative; area-duration: 8 positive + 2 negative).
- `scripts/simulate-iter88-impact.ts` — новый скрипт для mirror-симуляции iter 87 vs iter 88 patterns + diff на jewel/amulet/ring/belt.
- `scripts/analyze-iter88-other-bucket.ts` — новый скрипт для дампа всех family-keys в `other` + preliminary candidate patterns (использовался для проектирования AILMENTS/AREA_DURATION patterns).

**Ключевые файлы для будущих итераций (iter 89+):**
- `src/shared/mod-classifier.ts` — реализовать ещё 2-3 блока (buff-skills / meta-skills / wisps) для дальнейшего снижения other-bucket jewel.json.
