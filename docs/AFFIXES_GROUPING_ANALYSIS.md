...
| **P2** ✅ | hideLabel auto-suppression для блоков с 1 чипом | `ModList.tsx` (расширение iter 62 логики) | **done iter 62** | Меньше шума на малых категориях |
| **P3** | Приоритет тегов вместо first-match | `src/shared/mod-classifier.ts` | устарело (iter 96: теги больше не используются runtime — классификация через ETL `functionalCategory`) | — |
