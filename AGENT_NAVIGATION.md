...
> **Entry document.** Read this first. Current state: iter 76. **Все Known Issues закрыты** (KI-1, KI-2, KI-3). iter 76 (17 июня 2026): закрыл KI-3 (poe2db.tw OLD-forms стабильны >1 года, подтверждено `curl ... | grep -c "находимых в области"` = 107) + KI-2 data-level (ETL rerun с original OLD-form keys: waystone 302→156, tablet 86→84; 160+3 implicit-set bonus токенов отфильтровано, 5+5 implicit tokens добавлено). `WAYSTONE_IMPLICIT_SET_FAMILY_KEYS` reverted к original 4 ключам, `TABLET_IMPLICIT_SET_FAMILY_KEYS` reverted к `% увеличение количества находимых на карте путевых камней` (typo `%` — source verbatim). Tests restructured в `tests/etl/normalize.test.ts` (5 тестов): source-HTML verification (ключи не stale) + post-ETL JSON verification (filter сработал). Waystone test threshold `150-200` → `140-200`. Все 1157 тестов зелёные (1155 baseline + 2 новых source-HTML теста). iter 75 обнаружил KI-3 и обновил hardcoded keys к NEW-формам (code-level фикс без ETL rerun) — iter 76 reverted это, потому что OLD forms стабильны. iter 74: почищено 11 pre-existing lint-ошибок в 5 тест-файлах. iter 73 закрыл KI-1 (`?` tokenizer mismatch): `hasUnsupportedOptional()` detector + runtime warn в `matchQuotedGroup` + `unsupportedSyntax` flag в Oracle + reject в `iterative-optimizer`. Четыре атмосферных CSS-примитива в `index.css`: `.poe-panel-header` / `.poe-panel-header--inline`, `.poe-divider` / `.poe-divider--ornate`, `.poe-divider--banner`, `.btn-cta`. Фон — `atmosphere/bg.webp` + vignette. HomePage hero: lg+ — bas-relief backdrop + 2 xl+ side ghosts; <lg — `news-bg-center.webp` backdrop. SeoBlock: `hero-demon-blue.webp` декорация на правом краю при `[open]`, lg+ only. iter 68: `.poe-panel-header--inline` в JSX на 8 category pages; TopNav tab font 14px. **Pitfall 30 (resolved iter 73):** `?` tokenizer теперь детектится через `hasUnsupportedOptional()`, Oracle форсит `valid=false`, ETL `iterative-optimizer` отклоняет такие regex. **Pitfall 31 (resolved iter 76):** implicit-set family keys корректны (OLD forms, source-verbatim typo для tablet), ETL rerun применил фильтр — 163 implicit-set bonus токенов удалено, 10 implicit tokens добавлено. **Pitfall 32 (resolved iter 76):** poe2db.tw OLD forms подтверждены стабильные >1 года, ETL rerun завершён. Pitfall 22 + 28 + 29 актуальны.
