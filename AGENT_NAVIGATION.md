...
> **Entry document.** Read this first. Current state: iter 78. **Все Known Issues закрыты** (KI-1, KI-2, KI-3). iter 78 (18 июня 2026): Bug #8 Phase 1 — `useCategoryPage.ts` 1325 → 486 строк (63% reduction). Pure AST helpers extracted в новый файл `src/ui/hooks/category-ast-utils.ts` (890 строк): `getEffectiveRange`, `getEffectiveRangePerSlot`, `getPrefixForSlot`, `getSignPrefix`, `buildLiteralNode`, `pushLiteralsWithFamilyLogic`, `computeSuppressedExcludes`, `buildAstFromSelections`, `applyRuntimeYofication`. Все pure (no React, no side effects). `useCategoryPage.ts` re-exports `buildAstFromSelections` + `pushLiteralsWithFamilyLogic` + `applyRuntimeYofication` из нового файла — backward compat с existing tests (`tests/ui/buildAstFromSelections.test.ts`, `tests/ui/vendor-regex-equivalence.test.ts` импортируют из `@ui/hooks/useCategoryPage`). 2 lint fixes: (1) `react-hooks/set-state-in-effect` at useCategoryPage:1100 — removed redundant `setData(providedData)` + `setLoading(false)` в effect branch (useState initializers уже set эти values, effect branch только вызывал cascading renders); (2) `react-hooks/exhaustive-deps` at useCategoryPage:1022 — added `eslint-disable-next-line` с explanatory comment: `categoryId` intentionally в dep array для cache invalidation (каждая category page должна иметь свой filter store). Lint: 7 → 5 problems. Все 1157 тестов зелёные, `tsc -b` чистый. iter 77: Lint cleanup 44→7 problems (37 fixed в 14 файлах). **Bug #8 Phase 2 (deferred):** split `useCategoryPage` hook на 4 hooks (`useCategoryData`/`useRegexBuilder`/`useUrlSync`/`useFilterActions`) + fix 3 setState-in-effect в WaystonePage/JewelPage/TabletPage — требует API change affecting all 8 category pages (circular dependency: extraAstNodes зависит от local state, local state needs lazy init from filterStore, filterStore created by useCategoryPage which needs extraAstNodes). **Bug #13 (deferred):** `iterative-optimizer.ts:470` skip `.*[0-9][1-9]` — не 1-line fix, removing skip изменяет ETL output (public/generated/*.json), требует careful analysis + ETL rerun + diff verification. Четыре атмосферных CSS-примитива в `index.css`: `.poe-panel-header` / `.poe-panel-header--inline`, `.poe-divider` / `.poe-divider--ornate`, `.poe-divider--banner`, `.btn-cta`. Фон — `atmosphere/bg.webp` + vignette. HomePage hero: lg+ — bas-relief backdrop + 2 xl+ side ghosts; <lg — `news-bg-center.webp` backdrop. SeoBlock: `hero-demon-blue.webp` декорация на правом краю при `[open]`, lg+ only. iter 68: `.poe-panel-header--inline` в JSX на 8 category pages; TopNav tab font 14px. **Pitfall 30 (resolved iter 73):** `?` tokenizer теперь детектится через `hasUnsupportedOptional()`, Oracle форсит `valid=false`, ETL `iterative-optimizer` отклоняет такие regex. **Pitfall 31 (resolved iter 76):** implicit-set family keys корректны (OLD forms, source-verbatim typo для tablet), ETL rerun применил фильтр — 163 implicit-set bonus токенов удалено, 10 implicit tokens добавлено. **Pitfall 32 (resolved iter 76):** poe2db.tw OLD forms подтверждены стабильные >1 года, ETL rerun завершён. Pitfall 22 + 28 + 29 актуальны.
...
31. **Stale hardcoded implicit-set family keys — KI-2 (resolved iter 76).** `WAYSTONE_IMPLICIT_SET_FAMILY_KEYS` (4 ключа) и `TABLET_IMPLICIT_SET_FAMILY_KEYS` (1 ключ) в `scripts/etl/normalize.ts` теперь используют original OLD-form set (тот же, что и pre-iter-75):
    ...
    - **Не путать:** `WAYSTONE_IMPLICIT_SET_FAMILY_KEYS` — это FAMILY KEY для implicit-set бонусов waystone (отдельная концепция от `IMPLICIT_RANGE_UNRESTRICTED = [0, 350]` magic number в Bug #16).
...
33. **Bug #8 — `useCategoryPage` hook architecture (iter 78 Phase 1 done, Phase 2 deferred).** Hook имел 1325 строк в одном файле. iter 78 Phase 1 extracted pure AST helpers в `src/ui/hooks/category-ast-utils.ts` (890 строк) — `useCategoryPage.ts` теперь 486 строк (только React-часть). Phase 2 (split hook на 4 sub-hooks + fix 3 setState-in-effect в pages) требует API change:
    - **Circular dependency problem:** `extraAstNodes` (passed в `useCategoryPage`) зависит от page-local state (`corrupted`/`uncorrupted`/`delirious` в WaystonePage, `selectedTypes`/`selectedRarities`/`usesMin` в TabletPage, и т.д.). Local state хочется инициализировать из `filterStore` (созданного `useCategoryPage`) через `useState(() => filterStore.getExtraState(...))` для избежания `setState-in-effect`. Но `useCategoryPage` должен быть вызван AFTER `extraAstNodes`, который в свою очередь требует local state → circular.
    - **Planned fix (Phase 2):** split `useCategoryPage` на 4 hooks: (1) `useCategoryData({ categoryId, mergeCategories, customData })` → `{ data, loading, error }`; (2) `useFilterActions({ categoryId })` → `{ filterStore, selectedIds, ... , toggleToken, ... }` (creates filter store + URL restore + filter actions); (3) `useRegexBuilder({ data, selectedIds, extraAstNodes, ... })` → `{ regex, isRegexOverflow, regexParts, collapsedTokenIds }`; (4) `useUrlSync({ filterStore, deps })` → syncs store ↔ URL. Page component может вызвать `useFilterActions` BEFORE local `useState` (lazy init from filterStore), затем `useRegexBuilder` AFTER extraAstNodes computed.
    - **Migration impact:** все 8 category pages нужно обновить (Belt, Ring, Amulet, Relic, Waystone, Tablet, Jewel, Vendor). Каждый файл ~150-220 строк. Risk: UI regression — обязательно regression-test каждую page visual + behavior.
    - **Current lint state:** 5 problems (3 setState-in-effect в pages + 2 unfixable library warnings). Phase 2 закроет 3 setState-in-effect.
