...
> **Entry document.** Read this first. Current state: **iter 120** — фикс UI-багов: (1) scroll jump-to-top + jitter в `VirtualizedModList` (KI#6 — удалён `useLayoutEffect` с `virtualizer.measure()`, `ROW_ESTIMATES.subgroup` 120→60); (2) HomePage hero decorations (KI#7 — удалены `hero-bas-relief.webp` + `news-bg-center.webp`, side ghosts заменены на `hero-shaman.webp` + `hero-iva.webp` portrait full-body с CSS mask-image fade). iter 119 закрыл все priority-блоки сортировки (18 блоков, 312 family-keys, 100% coverage). Актуальный статус — в `STATUS.md`, история — в `worklog.md`, полный план сортировки — в `docs/AFFIX_ORDERING_PLAN.md`, полный UI-аудит — в `docs/UI_AUDIT.md`.
