...
> **Entry document.** Read this first. Current state: iter 74. **KI-2 (open):** `WAYSTONE_IMPLICIT_SET_FAMILY_KEYS` / `TABLET_IMPLICIT_SET_FAMILY_KEYS` в `scripts/etl/normalize.ts:380-390` устарели — 0 совпадений с `familyKey.ru` в текущем JSON, `isImplicitSetBonus()` молча no-op'ит. 3 `it.fails` теста в `tests/etl/normalize.test.ts` (Bug #15 / KI-2 блок) документируют ожидаемое поведение. Также в iter 74 почищено 11 pre-existing lint-ошибок в 5 тест-файлах (unused vars + explicit any → narrowing). iter 73 закрыл KI-1 (`?` tokenizer mismatch): `hasUnsupportedOptional()` detector + runtime warn в `matchQuotedGroup` + `unsupportedSyntax` flag в Oracle + reject в `iterative-optimizer`. Четыре атмосферных CSS-примитива в `index.css`: `.poe-panel-header` / `.poe-panel-header--inline`, `.poe-divider` / `.poe-divider--ornate`, `.poe-divider--banner`, `.btn-cta`. Фон — `atmosphere/bg.webp` + vignette. HomePage hero: lg+ — bas-relief backdrop + 2 xl+ side ghosts; <lg — `news-bg-center.webp` backdrop. SeoBlock: `hero-demon-blue.webp` декорация на правом краю при `[open]`, lg+ only. iter 68: `.poe-panel-header--inline` в JSX на 8 category pages; TopNav tab font 14px. **Pitfall 30 (resolved iter 73):** `?` tokenizer теперь детектится через `hasUnsupportedOptional()`, Oracle форсит `valid=false`, ETL `iterative-optimizer` отклоняет такие regex. Pitfall 22 + 28 + 29 + 31 актуальны.
...
31. **Stale hardcoded implicit-set family keys — KI-2 (open, iter 74).** `WAYSTONE_IMPLICIT_SET_FAMILY_KEYS` (4 ключа) и `TABLET_IMPLICIT_SET_FAMILY_KEYS` (1 ключ) в `scripts/etl/normalize.ts:380-390` устарели: 0 совпадений с актуальными `familyKey.ru` в `waystone.json` / `waystone-desecrated.json` / `tablet.json`. Причина — poe2db переформулировал тексты (например, `#% увеличение эффективности монстров` → `На #% больше эффективности монстров`). Фильтр `isImplicitSetBonus()` молча no-op'ит → implicit-set bonus токены НЕ выфильтровываются из mod-списков (хотя должны — они не searchable как mod text in-game).
    - **Тесты:** 3 `it.fails` в `tests/etl/normalize.test.ts` (Bug #15 / KI-2 блок). Каждый загружает соответствующий JSON, строит set из `familyKey.ru` (с нормализацией `## → #` + collapse whitespace, как в `isImplicitSetBonus`), и утверждает, что все хардкод-ключи присутствуют. Тесты сейчас FAIL (ожидаемо), `it.fails` инвертирует → suite остаётся зелёной.
    - **Когда фикс будет сделан** (отдельная итерация): обновить ключи по актуальным данным, запустить ETL, конвертировать `it.fails` → `it` (тесты должны начать проходить нормально). Если после фикса `it.fails` сообщает ошибку — это сигнал, что тесты прошли и их нужно переключить обратно на `it`.
