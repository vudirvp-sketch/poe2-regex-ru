...
> **Entry document.** Read this first. Current state: iter 72 (дедупликация ETL-утилит + удаление dead code + Known Issue KI-1 задокументирован). Четыре атмосферных CSS-примитива в `index.css`: `.poe-panel-header` / `.poe-panel-header--inline`, `.poe-divider` / `.poe-divider--ornate`, `.poe-divider--banner`, `.btn-cta`. Фон — `atmosphere/bg.webp` + vignette. HomePage hero: lg+ — bas-relief backdrop + 2 xl+ side ghosts; <lg — `news-bg-center.webp` backdrop. SeoBlock: `hero-demon-blue.webp` декорация на правом краю при `[open]`, lg+ only. iter 68: `.poe-panel-header--inline` в JSX на 8 category pages; TopNav tab font 14px. **Pitfall 30 (новый, iter 72):** `?` tokenizer mismatch — matcher допускает `?`, игра нет → Oracle FP risk. Pitfall 22 + 28 + 29 + 30 актуальны.
...
30. **`?` tokenizer mismatch — Oracle FP risk (iter 72, Known Issue KI-1):** `src/core/poe2-regex-matcher.ts:111-113` парсит `?` (вне `(?!`) как `optional` quantifier и matcher успешно его матчит. PoE2 in-game `?` **НЕ поддерживает** (verified Phase 7). Любой regex с `?` пройдёт Oracle-валидацию, но не сработает в игре. Тесты `tests/core/poe2-regex-matcher.test.ts:169-183, 932-940` залочили это поведение (комментарий "PoE2 does NOT support, but tokenizer should not break"). **Mitigation:** Generator (`compiler.ts` / `dp-factorizer.ts` / `trie-factorizer.ts`) `?` НЕ производит — в generated JSON паттернов с `?` нет. При модификации generator-логики НЕ добавлять `?`-паттерны. **Фикс отложен:** добавить runtime-warning в `matchQuotedGroup`/`matchPoE2Regex` при обнаружении `?` вне `(?!` контекста (с переработкой тестов на `expect(warn).toHaveBeenCalled()`). См. `STATUS.md` → KI-1.
