...
> **Entry document.** Read this first. Current state: iter 77. **Все Known Issues закрыты** (KI-1, KI-2, KI-3). iter 77 (18 июня 2026): Lint cleanup — 44→7 problems. Исправлено 37 lint issues (33 errors + 4 warnings) в 14 файлах: unused imports (5), `let`→`const` (3), unnecessary regex escapes (3), unused eslint-disable (1), `no-useless-assignment` в core (4: `compiler.ts` `excludeValues` init, `dp-factorizer.ts` + `trie-factorizer.ts` dead `placed = true`), `any`→proper types в scripts (20: `cheerio.Cheerio<unknown>`, Playwright `Browser`/`BrowserType`, `catch (err: unknown)` with type guards, `CategoryData`/`GameToken`/`OptimizationEntry` casts в `run-etl.ts`), `eslint.config.js` добавлен `no-unused-vars` ignore pattern `^_` (destructure-to-remove-key pattern). Осталось 7 problems: 4 `setState-in-effect` (WaystonePage/JewelPage/TabletPage/useCategoryPage — tied to Bug #8), 2 library warnings (`react-hooks/incompatible-library` для `useVirtualizer` в `VirtualizedModList.tsx` — нельзя фиксить без смены библиотеки), 1 `react-hooks/exhaustive-deps` warning в `useCategoryPage.ts` (tied to Bug #8). Все 1157 тестов зелёные, `tsc -b` чистый. iter 76 (17 июня 2026): закрыл KI-3 (poe2db.tw OLD-forms стабильны >1 года) + KI-2 data-level (ETL rerun с original OLD-form keys: waystone 302→156, tablet 86→84). iter 75 обнаружил KI-3 и обновил hardcoded keys к NEW-формам — iter 76 reverted это. iter 74: почищено 11 pre-existing lint-ошибок в 5 тест-файлах. iter 73 закрыл KI-1 (`?` tokenizer mismatch). Четыре атмосферных CSS-примитива в `index.css`: `.poe-panel-header` / `.poe-panel-header--inline`, `.poe-divider` / `.poe-divider--ornate`, `.poe-divider--banner`, `.btn-cta`. Фон — `atmosphere/bg.webp` + vignette. HomePage hero: lg+ — bas-relief backdrop + 2 xl+ side ghosts; <lg — `news-bg-center.webp` backdrop. SeoBlock: `hero-demon-blue.webp` декорация на правом краю при `[open]`, lg+ only. iter 68: `.poe-panel-header--inline` в JSX на 8 category pages; TopNav tab font 14px. **Pitfall 30 (resolved iter 73):** `?` tokenizer теперь детектится через `hasUnsupportedOptional()`, Oracle форсит `valid=false`, ETL `iterative-optimizer` отклоняет такие regex. **Pitfall 31 (resolved iter 76):** implicit-set family keys корректны (OLD forms, source-verbatim typo для tablet), ETL rerun применил фильтр — 163 implicit-set bonus токенов удалено, 10 implicit tokens добавлено. **Pitfall 32 (resolved iter 76):** poe2db.tw OLD forms подтверждены стабильные >1 года, ETL rerun завершён. Pitfall 22 + 28 + 29 актуальны.
