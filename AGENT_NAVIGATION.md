...
> **Entry document.** Read this first. Current state: **iter 105** (P2 second half — tablet sub-blocks. Новый режим `tablet-type-subblocks` в `classifyGroups()` с 19 sub-blocks: 3 per type (ritual-rewards/monsters/content, breach-monsters/rewards/content, delirium-mist/rewards/monsters, vaal-monsters/rewards/content, expedition-rewards/explosives/monsters) + 4 for generic (loot/monsters/encounters/player). TabletPage переключён со старого `tablet-type` (6 корзин) на новый режим (19 sub-blocks). Старый режим сохранён как legacy. iter 105 = tablet sub-blocks (P2 second half), runtime functional-classifier / ETL / JSON / схема / WaystonePage не тронуты. **1500/1500 тестов** зелёные, TSC 0 errors, ESLint 0 problems, ETL 11 fresh / 0 stale. Regex-движок: чистый TS, 0 npm-зависимостей. ETL: 1697 токенов, FN=0, FP=9463. Актуальный статус — в `STATUS.md`, история — в `worklog.md`.
...
- `src/shared/mod-classifier.ts` (~2180 строк) — iter 104 закрыл waystone sub-blocks, iter 105 закрыл tablet sub-blocks; остаются опциональный UI-toggle «режим сортировки» (P4) + low-priority waystone neutral-generic / tablet Разломы-keyword расширения.
...
- **P4 (next):** tier-aware sort toggle (UI-тумблер alpha vs tier-first в `CategoryControlPanel`). iter 99 сделал tier вторичным, но toggle не добавлен.
- **Опционально:** `sortKey?: number` в `FamilyGroup` + ETL заполнение для «по популярности внутри категории». **Опционально:** waystone neutral-generic (6 groups) — расширить POSITIVE_KEYWORDS для 5 desecrated Breach-adjacent mods. **Опционально:** tablet Разломы vs Бездна — расширить BREACH_KEYWORDS для 2 mods использующих «Разлом» (low-priority, текущая sub-block classification корректна).
