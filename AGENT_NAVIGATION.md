...
| `src/ui/pages/` | Category pages (Waystone, Tablet, Relic, Jewel, Vendor, Belt, Ring, Amulet, Home) | Category pages use `useCategoryPage`; VendorPage uses `useVendorPage` |
...
15. **VendorPage uses `useVendorPage` hook** — not `useCategoryPage`. Vendor-specific logic (VENDOR_PROPERTIES, buildVendorRegex, GROUP_COLORS) is in the hook
