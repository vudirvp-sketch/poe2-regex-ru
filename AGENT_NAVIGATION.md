...
pnpm test                 # Vitest (all tests) — current: 1431 passing
...
**Ключевые файлы для будущих итераций (P4 закрыт в iter 106; опциональные расширения):**
- `src/shared/mod-classifier.ts` (~2330 строк) — iter 104 закрыл waystone sub-blocks, iter 105 закрыл tablet sub-blocks, iter 106 закрыл P4 (tier-aware sort toggle). Остаются low-priority waystone neutral-generic / tablet Разломы-keyword расширения.
...
- `src/ui/components/ModList.tsx` (675 строк) — рендер sub-blocks (iter 106: `sortMode` prop пробрасывается в `classifyGroups`).
- `src/ui/components/VirtualizedModList.tsx` (832 строки) — virtualized рендер (iter 106: `sortMode` prop пробрасывается в `buildColumnRows`).
...
- **Опционально (next):** `sortKey?: number` в `FamilyGroup` + ETL заполнение для «по популярности внутри категории» — third sort mode (alpha / tier-first / popularity). **Опционально:** waystone neutral-generic (6 groups) — расширить POSITIVE_KEYWORDS для 5 desecrated Breach-adjacent mods. **Опционально:** tablet Разломы vs Бездна — расширить BREACH_KEYWORDS для 2 mods использующих «Разлом» (low-priority, текущая sub-block classification корректна). **Опционально:** visual indicator (tier-colored left border) для S-tier модов в tier-first mode.
