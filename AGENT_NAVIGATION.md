...
> **Entry document.** Read this first. Current state: **iter 122** — cleanup неиспользуемых atmosphere assets + интеграция faf.png в SeoBlock. iter 121 закрыл ре-фикс hero decorations (KI#7). iter 122: (1) удалены 4 stale webp (`hero-horned-warrior`, `hero-monster-red`, `hero-bas-relief`, `news-bg-center`) + dead script `scripts/optimize_hero_images.py` — все были помечены "no longer referenced" ещё в iter 120; (2) faf.png (1672×941, dark-fantasy арт) → `public/atmosphere/seo-atmosphere.webp` (1600×900, q85, 146 KB), добавлен как широкий landscape backdrop в SeoBlock (`.home-seo-atmosphere` класс, opacity 0.18, `mix-blend-screen`, fade bottom 40%, lg+ only, виден при раскрытом `<details>`). DOM order в SeoBlock: atmosphere → demon → content (demon paints поверх atmosphere, оба под content). iter 119 закрыл все priority-блоки сортировки (18 блоков, 312 family-keys, 100% coverage). Актуальный статус — в `STATUS.md`, история — в `worklog.md`, полный план сортировки — в `docs/AFFIX_ORDERING_PLAN.md`, полный UI-аудит — в `docs/UI_AUDIT.md`.
