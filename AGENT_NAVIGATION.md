...
> **Entry document.** Read this first. Current state: **iter 108 + post-fix audit** (фикс вложенных кавычек в OR-регексах для токенов с `regexPrefixContext` без `regexExclude`. `normalizeAst` в `src/core/compiler.ts` расширен: добавлена ветка для `AND(LITERAL..., LITERAL...)` без EXCLUDE внутри OR — LITERALs мержатся через `.*` bridge в один `LITERAL("A.*B.*...")`. До фикса такой AND компилировался в `"ctx" "regex"` (cross-block AND), что внутри внешнего OR-quote давало **вложенные кавычки** → PoE2 returning zero matches (rule B0). Фикс также делает семантику **более корректной**: same-block AND вместо cross-block AND — именно так и задумывался `regexPrefixContext`. Затронуты: tablet (2 токена, включая `mod_by2ufv` «...провал, вплоть до 100%»), amulet/jewel/ring (minion-моды с `имеют`), relic (boss-моды с `Боссы получают`/`Боссы наносят`). 10 новых регрессионных тестов в `tests/core/optimizer.test.ts` в `compile: AND-in-OR with EXCLUDE → anchored lookahead` describe block. **Post-fix exhaustive audit** (4 сценария × 10 категорий × 1697 токенов + 543 opt-table entries) — 0 критических нарушений (B0/NESTED/EMPTY). Аудит-скрипт: `/home/z/my-project/scripts/audit-corrected.ts`. **1543/1543 тестов** зелёные (+10 vs iter 107), TSC 0 errors, ESLint 0 problems, ETL 11 fresh / 0 stale. Regex-движок: чистый TS, 0 npm-зависимостей. ETL: 1697 токенов, FN=0, FP=9463. Актуальный статус — в `STATUS.md`, история — в `worklog.md`.
