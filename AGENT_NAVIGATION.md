...
> **Entry document.** Read this first. Current state: **iter 126** — фикс KI#10: ambiguous suffix FP для `Редкость предметов`. Пользователь сообщил: iter 125 fixed regex `"едкость.*\+[2-9][0-9]%|едкость.*\+\d{3,}%" "ивность.*\+[2-9][0-9]%|ивность.*\+\d{3,}%"` ВСЁ ЕЩЁ подсвечивал waystone с `Редкость предметов: +11%` (FP). По гипотезе пользователя, на waystone 2-4 имплицита, и `едкость` совпадает с несколькими типами редкости (предметов + монстров). Root cause: токен `waystone.implicit.item_rarity` использовал regex `'едкость'` (7 chars) — слишком общий, матчит любой текст с `едкость` substring, включая гипотетическую `Редкость монстров` (если такая implicit есть в игре, но не в нашей БД). Фикс: заменить regex на `'едкость предметов'` (12 chars, literal space) — уникально идентифицирует `Редкость предметов`. Применено через `scripts/etl/i18n-overrides.json` (override mechanism) + прямой patch `public/generated/waystone.json` + `waystone-desecrated.json`. После фикса: `"едкость предметов.*\+[2-9][0-9]%|едкость предметов.*\+\d{3,}%" "ивность.*\+[2-9][0-9]%|ивность.*\+\d{3,}%"` (107 chars ≤ 250). +24 новых теста в `tests/core/iter126-ki10-rarity-disambiguation.test.ts` (5 секций). KI#11 (NEW, MONITORING): если FP вызван cross-block `.*` (а не ambiguous suffix), фикс iter 126 НЕ поможет — потребуется `literalBridge` поле в AST + compiler changes. Ждать user verification по KI#10 — если FP сохранится, эскалировать до KI#11 fix. iter 125: фикс `(A|B|C) after .* bridge` для reversed RANGE. iter 124: cleanup stale `DELETIONS-iter123.txt`. iter 122: SeoBlock atmosphere. iter 121: ре-фикс hero decorations (KI#7). KI#7/KI#8/KI#9/KI#11 — awaiting user verification/monitoring. Актуальный статус — в `STATUS.md`, история — в `worklog.md`, полный план сортировки — в `docs/AFFIX_ORDERING_PLAN.md`, полный UI-аудит — в `docs/UI_AUDIT.md`.
...
37. **`.*` cross-block hypothesis (KI#11, UNVERIFIED, iter 126).** Phase 7 in-game verification (см. `docs/IN_GAME_TESTS.md`) установила, что `.*` НЕ пересекает block boundaries в PoE2 in-game regex. Симулятор (`src/core/poe2-regex-matcher.ts`) моделирует это поведение (`getItemSearchBlocks` → each implicit/mod is a separate block, `.*` restricted to single block). **Hypothesis (iter 126):** Возможно, in-game `.*` ДЕЙСТВИТЕЛЬНО пересекает blocks для items с multiple implicits (waystone scenario). Если hypothesis верна, iter 126 KI#10 fix (`едкость предметов`) НЕ уберёт FP — `.*` между `предметов` и `+XX%` всё ещё может пересечь blocks. **Verification plan:** Пользователь тестирует iter 126 fixed regex в игре — если FP сохраняется, hypothesis подтверждается. **Mitigation plan (если KI#11 подтвердится):** 1. Добавить `literalBridge?: string` поле в AST RANGE node. 2. В `category-ast-utils.ts` detect literal bridge из template (для `Редкость предметов: +##%` → `literalBridge = " предметов: "`). 3. В `compiler.ts` использовать `literalBridge` вместо `.*` между suffix и numRegex — produces `едкость предметов: \+XX%` (no `.*`). 4. Обновить симулятор для опционального моделирования cross-block `.*` (для тестирования KI#11 fix). **Action:** Ждать user verification по KI#10 fix в iter 126.
