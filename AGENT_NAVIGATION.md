...
| `src/ui/pages/` | Category pages (Waystone, Tablet, Relic, Jewel, Vendor, Belt, Ring, Amulet, Home) | Category pages use `useCategoryPage`; VendorPage uses `useVendorPage` (migrating to `useCategoryPage` with `customData`) |
| `src/ui/components/` | Shared UI — FilterChip, VendorChip, ModList, VirtualizedModList, RegexOutput, CategoryControlPanel, ProfilePanel | FilterChip = per-mod toggle, VendorChip = vendor property switch (will merge with FilterChip after N1) |
...
15. **VendorPage uses `useVendorPage` hook** — not `useCategoryPage`. Migration in progress: `customData` option added to `useCategoryPage`, vendor-adapter fixed. Final switch requires visual verification
