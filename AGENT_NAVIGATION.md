...
> **Entry document.** Read this first. Current state: **iter 121** — ре-фикс HomePage hero decorations (KI#7). iter 120 fix был неполным: side ghosts были внутри hero block с `overflow-hidden` + center-anchor → голова и ноги обрезаны (виден только торс); плюс изображения заперты в `max-w-4xl` (896px), не у краёв экрана. iter 121: Layout.tsx — `<main>` получил `relative`; HomePage.tsx — side ghosts вынесены в Fragment (siblings max-w-4xl), anchored `absolute top-0 left-0`/`right-0` (края экрана ниже TopNav), `h-[80vh] max-h-[720px]` (90% natural 800px), opacity 0.20, content в `relative z-10`; index.css — bottom fade `#000 75%` (вместо 55%), horizontal fade на INNER edge (вместо OUTER — баг iter 120). iter 120 также пофиксил scroll jump-to-top + jitter (KI#6 — корректный фикс, остаётся в силе). iter 119 закрыл все priority-блоки сортировки (18 блоков, 312 family-keys, 100% coverage). Актуальный статус — в `STATUS.md`, история — в `worklog.md`, полный план сортировки — в `docs/AFFIX_ORDERING_PLAN.md`, полный UI-аудит — в `docs/UI_AUDIT.md`.
