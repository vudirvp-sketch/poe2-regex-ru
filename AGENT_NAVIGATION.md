...
> **Entry document.** Read this first. Current state: iter 75. **KI-3 (open, iter 75):** poe2db.tw откатил тексты модов к OLD-формам между 16 и 17 июня. Current `public/generated/*.json` (от 16 июня) содержит NEW-формы; fresh fetch даёт OLD-формы. ETL rerun BLOCKED — нельзя применить KI-2 фикс без перегенерации JSON с OLD-формами (сломает ~40 тестов). **KI-2 (code-fixed, iter 75):** `WAYSTONE_IMPLICIT_SET_FAMILY_KEYS` (4→2 ключа) и `TABLET_IMPLICIT_SET_FAMILY_KEYS` (typo fix `%` → `#%`) в `scripts/etl/normalize.ts` обновлены по актуальным `familyKey.ru` из current JSON. 3 теста конвертированы `it.fails` → `it` в `tests/etl/normalize.test.ts`. Data-level фикс (ETL rerun) blocked by KI-3. iter 74: почищено 11 pre-existing lint-ошибок в 5 тест-файлах. iter 73 закрыл KI-1 (`?` tokenizer mismatch): `hasUnsupportedOptional()` detector + runtime warn в `matchQuotedGroup` + `unsupportedSyntax` flag в Oracle + reject в `iterative-optimizer`. Четыре атмосферных CSS-примитива в `index.css`: `.poe-panel-header` / `.poe-panel-header--inline`, `.poe-divider` / `.poe-divider--ornate`, `.poe-divider--banner`, `.btn-cta`. Фон — `atmosphere/bg.webp` + vignette. HomePage hero: lg+ — bas-relief backdrop + 2 xl+ side ghosts; <lg — `news-bg-center.webp` backdrop. SeoBlock: `hero-demon-blue.webp` декорация на правом краю при `[open]`, lg+ only. iter 68: `.poe-panel-header--inline` в JSX на 8 category pages; TopNav tab font 14px. **Pitfall 30 (resolved iter 73):** `?` tokenizer теперь детектится через `hasUnsupportedOptional()`, Oracle форсит `valid=false`, ETL `iterative-optimizer` отклоняет такие regex. **Pitfall 31 (code-fixed iter 75, data-blocked by KI-3):** implicit-set family keys обновлены, но ETL rerun заблокирован. Pitfall 22 + 28 + 29 актуальны.
...
31. **Stale hardcoded implicit-set family keys — KI-2 (code-fixed iter 75, data-blocked by KI-3).** `WAYSTONE_IMPLICIT_SET_FAMILY_KEYS` и `TABLET_IMPLICIT_SET_FAMILY_KEYS` в `scripts/etl/normalize.ts` обновлены по актуальным `familyKey.ru` из current JSON (NEW-формы от 16 июня). iter 75 изменения:
    - **Waystone keys (4→2):** удалены `На #% больше находимых в области путевых камней` и `На #% больше редкости находимых в этой области предметов` (нет совпадений в current JSON — poe2db убрал mod-form токены для этих статов, оставил только implicit-form); `#% увеличение эффективности монстров` → `На #% больше эффективности монстров` (reworded poe2db); `На #% больше размера групп монстров` удалён (нет совпадений). Добавлен `#% увеличение количества путевых камней, находимых в области` (new mod-form).
    - **Tablet key (1, typo fix):** `% увеличение количества находимых на карте путевых камней` → `#% увеличение количества находимых на карте путевых камней` (добавлен `#` перед `%`).
    - **Тесты:** 3 теста в `tests/etl/normalize.test.ts` конвертированы `it.fails` → `it`. Waystone и tablet тесты проверяют, что каждый ключ присутствует в соответствующем `familyKey.ru` set. Waystone-desecrated тест реструктурирован: проверяет, что waystone-desecrated.json НЕ содержит mod-form implicit-set bonus токенов (filter no-op для этой категории — только implicit tokens, никаких mod-form дубликатов).
    - **Data-level фикс BLOCKED by KI-3:** ETL rerun невозможен, потому что poe2db.tw откатил тексты к OLD-формам (см. KI-3 в STATUS.md). Current JSON всё ещё содержит implicit-set bonus токены (15 в waystone, 1 в tablet), потому что ETL не запускался. Когда KI-3 будет решён, ETL перегенерирует JSON и токены исчезнут.
...
32. **poe2db.tw text form revert — KI-3 (open, iter 75).** Между 16 и 17 июня poe2db.tw откатил формулировки текстов модов к OLD-формам. Current `public/generated/*.json` (от 16 июня, `pnpm etl:fresh`) содержит NEW-формы (e.g., `#% увеличение количества путевых камней, находимых в области`, `На #% больше эффективности монстров`). Свежий fetch с poe2db.tw (17 июня) даёт OLD-формы (e.g., `На #% больше находимых в области путевых камней`, `#% увеличение эффективности монстров`).
    - **Симптом:** `npx tsx scripts/run-etl.ts` (даже с `--fresh`) производит waystone.json с 302 токенами (vs 156 в current JSON) и OLD-формами familyKey. Tablet.json: 84→86 токенов. Многие тесты сломаются (cross-validation thresholds 150-200 для waystone, regex content matching).
    - **Влияние на KI-2:** нельзя запустить ETL для применения фильтра implicit-set bonus токенов — ETL перегенерирует JSON с OLD-формами, что сломает тесты. KI-2 code-fixed (ключи обновлены под NEW-формы), но data-level фикс отложен.
    - **Решение** (отдельная итерация): либо (a) обновить хардкод-ключи KI-2 к OLD-формам + перегенерировать JSON + обновить test thresholds, либо (b) дождаться пока poe2db.tw снова вернёт NEW-формы. Требует анализа что правильнее: OLD или NEW формы (возможно, NEW-формы были intermediate состоянием poe2db, а OLD-формы — stable).
