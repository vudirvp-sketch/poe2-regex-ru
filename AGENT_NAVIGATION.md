...
> **Current Iteration:** Iteration 17 (Jewel Type Sub-Grouping + Classification Accuracy + TabletPage Refactor) — COMPLETE.
...
> **Session 23:** TabletPage PageStateWrapper refactor (inline loading/error→shared component), jewel-type groupMode in mod-classifier + ModList (visual sub-grouping within origin sections), static lookup table for jewel classification (~210 familyKeys from poe2db → 100% accuracy vs ~80% with heuristics alone), ETL re-run (51 overrides).
...
| 17: Jewel Type Sub-Grouping + Classification + Refactor | ✅ Complete | TabletPage PageStateWrapper refactor, jewel-type groupMode + visual sub-grouping in ModList, static lookup table for jewel classification accuracy, ETL re-run |
