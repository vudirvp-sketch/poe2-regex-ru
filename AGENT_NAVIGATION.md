...
> **Entry document.** Read this first. Current state: iter 71 (3 leftover atmospheric WebP интегрированы). Четыре атмосферных CSS-примитива в `index.css`: `.poe-panel-header` / `.poe-panel-header--inline` (gold filigree rim по образу `title-bg-4x.webp`), `.poe-divider` / `.poe-divider--ornate` (bg-2x texture), `.poe-divider--banner` (iter 71 — `early-access-banner.webp`), `.btn-cta` (warm metallic + crimson glow по образу `early-access-button-underlay.webp`) — заменяет холодный `bg-btn-primary` на Copy-кнопках RegexOutput. Фон — `atmosphere/bg.webp` + vignette. HomePage hero: lg+ — bas-relief backdrop (`mix-blend-screen` 0.18) + 2 xl+ side ghosts; <lg — `news-bg-center.webp` backdrop (iter 71, `mix-blend-screen` 0.14). SeoBlock: `hero-demon-blue.webp` декорация на правом краю, появляется только при `[open]`, opacity 0.10, lg+ only (iter 71). iter 68: `.poe-panel-header--inline` применён в JSX на 8 category pages; TopNav tab font 14px. Pitfall 22 + 28 + 29 актуальны.
