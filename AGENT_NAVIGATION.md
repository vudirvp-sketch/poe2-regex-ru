...
> **Entry document.** Read this first. Current state: **iter 119** — расширение систематической сортировки аффиксов внутри функциональных блоков на `rage-charges` (4 family-keys) + `runes-barrier` (4 family-keys) + `penetration` (3 family-keys). **Все priority-блоки закрыты** (18 из 24 functional blocks, 312 family-keys, 100% coverage). iter 112 внедрил инфраструктуру sortKey + 4 блока правил (resistances/attributes/minions/ailments). iter 113 добавил `damage-type` (47 family-keys). iter 114 добавил `defence-stats` (28 family-keys). iter 115 добавил `resources` (29 family-keys). iter 116 добавил `weapon-specific` (24 family-keys) + `flasks` (16 family-keys). iter 117 добавил `offence-speed` (12 family-keys) + `crit` (9 family-keys) + `buff-skills` (7 family-keys). iter 118 добавил `skill-levels` (10 family-keys) + `area-duration` (8 family-keys) + `meta-skills` (6 family-keys). Актуальный статус — в `STATUS.md`, история — в `worklog.md`, полный план сортировки — в `docs/AFFIX_ORDERING_PLAN.md`, полный UI-аудит — в `docs/UI_AUDIT.md`.
