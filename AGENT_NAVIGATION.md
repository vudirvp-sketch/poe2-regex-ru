...
> **Entry document.** Read this first. Current state: **iter 127** — аудит KI#10-pattern в других категориях + фикс KI#12 (tier-hardcoded regex для 7 single-# relic tokens). Пользователь подтвердил iter 126 fix работает in-game → **KI#11 (cross-block `.*` hypothesis) ОПРОВЕРГНУТА**, KI#10 → VERIFIED. iter 127 аудит: waystone implicits / explicits SAFE (KF#10 был единственным ambiguous suffix FP). Найден NEW KI#12: 7 relic Sanctum tokens с single-`#` template имели tier-hardcoded regex (`'на 6%'`, `'на 4%'`, `'ат: 2'`, etc.) — содержали digit value из rawText, матчит ONLY tier 1, FN для tiers 2+. Family-level opt entries использовали FIRST (alphabetically) token's regex → tier-hardcoded. Фикс: explicit overrides в `i18n-overrides.json` для всех 7 tokens (tier-agnostic regex matching `##` siblings) + patch `relic.json` (7 token regexes + 4 family opt entries + delete 4 broken cross-family entries). +19 тестов в `tests/core/iter127-ki12-tier-hardcoded-regex.test.ts` (7 секций). Audit scripts: `scripts/audit-ambiguous-suffixes.py` (KI#10-pattern) + `scripts/audit-tier-hardcoded-regex.py` (KI#12-pattern). 1958/1958 tests. iter 126: фикс KI#10 (VERIFIED in-game iter 127). iter 125: фикс `(A|B|C) after .* bridge` для reversed RANGE. KI#7/KI#8/KI#9 — awaiting user verification/monitoring. Актуальный статус — в `STATUS.md`, история — в `worklog.md`, полный план сортировки — в `docs/AFFIX_ORDERING_PLAN.md`, полный UI-аудит — в `docs/UI_AUDIT.md`.
