...
> **Текущее состояние:** iter 183 — state-features для `/timeless-jewel` (URL-sync через `#tj=` + ProfilePanel + AtlasSelectedBasket). iter 182 — UX-fix KI#57 + visual density. iter 181 — UI fixes (KI#55/KI#56 + rename). iter 180 — SEO technical fixes. iter 179 — README rewrite. iter 178 — полировка `/timeless-jewel`. iter 175–177 — DONE ✅.
...
> **Базовые проверки:** `npx tsc -b`, `npx eslint .`, `npx vitest run` (2455 passed | 5 skipped), `npx vite build`. Актуальный статус — в `STATUS.md`, история — в `worklog.md`.
...
| `src/store/` | Zustand stores — filter-store, profile-store, url-sync, **atlas-state-sync** (iter 183) | Import from `@shared`, `@core` |
...
| `src/ui/components/AtlasSelectedBasket.tsx` | **iter 183 — NEW.** Minimal selected-basket for `/timeless-jewel`. Чипы с именами atlas-нод + ✗ для удаления. Cap=20 с «+N ещё» (mirrors `SELECTED_BASKET_CAP`). Без family-grouping (atlas-ноды не имеют familyKey/affix). Empty state «Выберите ноды из списка». | `/timeless-jewel` only — NOT for item-category pages |
...
28. **Atlas Timeless Jewel — ОТДЕЛЬНЫЙ pipeline (iter 176, iter 178 polish, iter 183 state-features):** Категория `/timeless-jewel` НЕ использует `useCategoryPage`, `filter-store`, `compiler.ts`, `optimizer.ts`, `ast.ts` или `ModList`/`VirtualizedModList`. Причины: (1) Atlas-семантика OR-only (см. pitfall #27) — существующий engine заточен под AND/NOT/ranges; (2) `GameToken` schema избыточна (нет ranges/familyKey/affix/genderForms). Используются: `AtlasNodeToken` (новый минимальный тип), `atlas-jewel-loader.ts` (отдельный loader), `atlas-regex-builder.ts` (новый `buildAtlasRegex()` — OR-only + alphabetical sort + dedupe + overflow split), `AtlasNodeList` (плоский список с чекбоксами/иконками/описаниями), `AtlasSelectedBasket` (iter 183 — minimal basket без family-grouping), `atlas-state-sync.ts` (iter 183 — URL hash sync через `#tj=` prefix), `TimelessJewelPage` (страница). Переиспользуется только `RegexOutput` (props `filterStore` передаётся как `null` — share-кнопка disabled), `MobileRegexBar` (iter 178 — sticky-bottom на mobile), и `ProfilePanel` (iter 183 — generic over `currentFilterData`, работает с любым JSON-serializable state). **iter 178:** (a) иконка навигации `jewel` → `timeless-jewel` (отдельный `public/icons/timeless-jewel.png` 128×128 RGBA — пользовательская, залитая в commit `8143975`); (b) title `Особые самоцветы` → `Вневременные самоцветы`; (c) 15 atlas-node иконок self-hosted в `public/icons/atlas-nodes/`; (d) `AtlasNodeTokenSchema.iconUrl` принимает и http(s) URL, и относительные пути `icons/...` через `.refine()`; (e) `/timeless-jewel` добавлен в `scripts/prerender.ts` (routes + navLinks) и `public/sitemap.xml`. **iter 183:** (a) URL-sync через `#tj=<lz-string>` prefix (ОТЛИЧАЕТСЯ от filter-store `#q=` — страницы не парсят чужой хэш); (b) ProfilePanel wired с `currentFilterData = serializeAtlasState(jewel, ids)` и `onRestore = deserializeAtlasState`; (c) `AtlasSelectedBasket` рендерится в правом aside (desktop) + в mobile-section (mobile). **iter 183 НЕ делает:** cross-tab persistence через localStorage (analog `poe2:favorites:<cat>` / `poe2:uistate:<cat>`) — deferred to iter 184. При модификации `/timeless-jewel` — НЕ трогать `/jewel` и наоборот. Парсер данных — отдельный скрипт `scripts/etl/parse-timeless-jewel.ts` (НЕ в `run-etl.ts`).
...
| `docs/ATLAS_JEWEL_PLAN.md` | iter 175–178 — план + реализация категории `/timeless-jewel`. iter 178 DONE (rename + иконка + MobileRegexBar + SEO + self-host icons). iter 179 DONE (README + docs cleanup). iter 183 DONE (state-features: URL-sync `#tj=` + ProfilePanel + AtlasSelectedBasket). iter 184+ — cross-tab persistence (localStorage). |
