...
> **Entry document.** Read this first. Current state: **iter 129** — cleanup + stabilisation. (1) Cleanup dead BTS-related regex patterns в `src/shared/mod-classifier.ts` (6 patterns удалены из 5 констант: `POSITIVE_KEYWORDS`, `NEGATIVE_KEYWORDS`, `POSITIVE_LOOT_PATTERNS`, `NEGATIVE_MONSTER_MODIFIERS_PATTERNS`, `WAYSTONE_A_PREFIX`). Все эти patterns матчили только BTS-токены, отфильтрованные на ETL в iter 128 (KI#13). Удалены 4 соответствующих теста в `tests/shared/mod-classifier.test.ts`. 1992→1988 tests. (2) KI#7 (HomePage hero decorations, iter 121) → VERIFIED: side-ghost portraits (`hero-shaman.webp` + `hero-iva.webp`), CSS `.hero-side-ghost` (index.css:609-662), build passes. (3) KI#8 (SeoBlock atmosphere, iter 122) → VERIFIED: `seo-atmosphere.webp` + `hero-demon-blue.webp`, CSS `.home-seo-atmosphere` (index.css:1090-1126), build passes. (4) UI Refactor Plan в `docs/UI_REFACTOR_PLAN.md` (~530 строк, 5 фаз, без реализации). KI#9 — monitoring (MULTI_RANGE slot N>0). KI#10-KI#13 закрыты. Актуальный статус — в `STATUS.md`, история — в `worklog.md`, полный план UI-рефакторинга — в `docs/UI_REFACTOR_PLAN.md`, полный план сортировки — в `docs/AFFIX_ORDERING_PLAN.md`, полный UI-аудит — в `docs/UI_AUDIT.md`.
