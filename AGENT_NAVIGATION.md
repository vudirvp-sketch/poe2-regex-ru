...
> **Entry document.** Read this first. Current state: **iter 124** — cleanup stale `DELETIONS-iter123.txt` instruction file (список инструкций устарел — целевые файлы `DELETIONS-iter{121,122}.txt` уже удалены в iter 123; iter 124 удаляет сам список инструкций, по аналогии с iter 121 → `DELETIONS-iter100.txt` и iter 123 → `DELETIONS-iter{121,122}.txt`). iter 123: cleanup `DELETIONS-iter{121,122}.txt`. iter 122: cleanup 4 неиспользуемых atmosphere webp + dead script `optimize_hero_images.py` + интеграция `faf.png` как `seo-atmosphere.webp` (1600×900, q85, 146 KB) — широкий landscape backdrop в SeoBlock (`.home-seo-atmosphere` класс, opacity 0.18, `mix-blend-screen`, fade bottom 40%, lg+ only, виден при раскрытом `<details>`). DOM order в SeoBlock: atmosphere → demon → content. iter 121: ре-фикс hero decorations (KI#7). iter 119 закрыл все priority-блоки сортировки (18 блоков, 312 family-keys, 100% coverage). KI#7 (hero) и KI#8 (SeoBlock atmosphere) — awaiting user visual verification. Актуальный статус — в `STATUS.md`, история — в `worklog.md`, полный план сортировки — в `docs/AFFIX_ORDERING_PLAN.md`, полный UI-аудит — в `docs/UI_AUDIT.md`.
