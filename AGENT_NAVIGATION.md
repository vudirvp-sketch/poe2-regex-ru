...
| `public/atmosphere/` | iter 65: PoE2-themed texture assets used by `index.css`. Files: `bg.webp` (body background texture), `bg-2x.webp` (`.poe-divider--ornate` border texture), `title-bg-4x.webp` + `early-access-button-underlay.webp` (visual reference only — not referenced from CSS; `.poe-panel-header` and `.btn-cta` are pure CSS reinterpretations of their geometry). | Old `public/bg-forest.webp` / `public/bg-forest-mobile.webp` are kept for cached-URL backward-compat but no longer referenced. |
