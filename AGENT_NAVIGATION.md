...
> **Entry document.** Read this first. Current state: iter 69 (HomePage hero decorations). Три атмосферных CSS-примитива в `index.css`: `.poe-panel-header` / `.poe-panel-header--inline` (gold filigree rim по образу `title-bg-4x.webp`), `.poe-divider` / `.poe-divider--ornate` (bg-2x texture), `.btn-cta` (warm metallic + crimson glow по образу `early-access-button-underlay.webp`) — заменяет холодный `bg-btn-primary` на Copy-кнопках RegexOutput. Фон — `atmosphere/bg.webp` + vignette. HomePage hero на lg+/xl+ имеет 3 атмосферные декорации: bas-relief backdrop (`mix-blend-screen`) + 2 side ghosts. iter 68: `.poe-panel-header--inline` применён в JSX на 8 category pages; TopNav tab font 14px. Pitfall 22 + 28 + 29 актуальны.
...
| `public/atmosphere/` | PoE2-themed texture + hero decoration assets. **iter 65**: `bg.webp` (body bg texture), `bg-2x.webp` (`.poe-divider--ornate` border texture), `title-bg-4x.webp` + `early-access-button-underlay.webp` (visual reference only — `.poe-panel-header` / `.btn-cta` are pure CSS reinterpretations). **iter 67**: `early-access-banner.webp` + `news-bg-center.webp` (deferred — iter 70 candidates). **iter 69**: 4 hero decoration WebPs (PNG → WebP q85 via `scripts/optimize_hero_images.py`): `hero-bas-relief.webp` (backdrop, lg+, `mix-blend-screen` opacity 0.18), `hero-horned-warrior.webp` (L side ghost, xl+, opacity 0.28), `hero-monster-red.webp` (R side ghost, xl+, opacity 0.28), `hero-demon-blue.webp` (deferred — iter 70 candidate). All 3 active hero images referenced from `HomePage.tsx`. | Old `public/bg-forest.webp` / `public/bg-forest-mobile.webp` kept for cached-URL backward-compat (iter 70 candidate: remove). |
...
29. **Atmospheric CSS primitives (iter 65, UI Phase 11) — three reusable classes in `index.css`.** All three are PURE CSS reinterpretations of PoE2 trade-UI asset pack geometry — the source `.webp` files are visual references, not directly referenced by `background-image` (except `bg.webp` for body + `bg-2x.webp` for `.poe-divider--ornate`).
    ...
    - **Body background swap (iter 65):** `bg-forest.webp` → `atmosphere/bg.webp` + `linear-gradient(rgba(13,11,9,0.78)→rgba(7,5,3,0.92))` warm dim + `radial-gradient(vignette)` darker corners. The `bg-forest.webp` / `bg-forest-mobile.webp` files are KEPT in `public/` for cached-URL backward-compat (browsers/CDN may still have them cached) — do NOT delete them yet. Re-evaluate after a release cycle.
