...
> **Entry document.** Read this first. Current state: iter 79. **Все Known Issues закрыты** (KI-1, KI-2, KI-3). iter 79 (18 июня 2026): Bug #8 Phase 2 — split `useCategoryPage` на 3 sub-hooks (`useFilterStore`/`useCategoryData`/`useRegexBuilder`) + fixed 3 setState-in-effect errors в WaystonePage/JewelPage/TabletPage. `useCategoryPage` теперь составляет эти 3 hook + URL sync inline (tightly coupled к 6 useState values — extract не оправдан). `useCategoryPage(config)` принимает optional `config.filterStore: FilterStoreHook` — pages с extraAstNodes-from-local-state (Waystone/Jewel/Tablet) вызывают `useFilterStore(categoryId)` BEFORE local `useState`, lazy-init из extraState, потом передают `filterStore` в `useCategoryPage`. Это ломает circular dependency (см. Pitfall 33). 5 unaffected pages (Belt/Ring/Amulet/Relic/Vendor) НЕ менялись — backward compat. Lint: 5 → 2 problems (3 setState-in-effect errors fixed; 2 unfixable library warnings: `useVirtualizer` returns non-memoizable functions). Все 1157 тестов зелёные, `tsc -b` чистый. iter 78: Bug #8 Phase 1 — pure AST helpers extracted в `src/ui/hooks/category-ast-utils.ts` (890 строк); `useCategoryPage.ts` 1325 → 486 строк (re-exports preserved для tests). iter 77: Lint cleanup 44→7 problems. **Bug #13 (deferred):** `iterative-optimizer.ts:470` skip `.*[0-9][1-9]` — не 1-line fix, removing skip изменяет ETL output (public/generated/*.json), требует careful analysis + ETL rerun + diff verification. Четыре атмосферных CSS-примитива в `index.css`: `.poe-panel-header` / `.poe-panel-header--inline`, `.poe-divider` / `.poe-divider--ornate`, `.poe-divider--banner`, `.btn-cta`. Фон — `atmosphere/bg.webp` + vignette. HomePage hero: lg+ — bas-relief backdrop + 2 xl+ side ghosts; <lg — `news-bg-center.webp` backdrop. SeoBlock: `hero-demon-blue.webp` декорация на правом краю при `[open]`, lg+ only. iter 68: `.poe-panel-header--inline` в JSX на 8 category pages; TopNav tab font 14px. **Pitfall 30 (resolved iter 73):** `?` tokenizer теперь детектится через `hasUnsupportedOptional()`, Oracle форсит `valid=false`, ETL `iterative-optimizer` отклоняет такие regex. **Pitfall 31 (resolved iter 76):** implicit-set family keys корректны (OLD forms, source-verbatim typo для tablet), ETL rerun применил фильтр — 163 implicit-set bonus токенов удалено, 10 implicit tokens добавлено. **Pitfall 32 (resolved iter 76):** poe2db.tw OLD forms подтверждены стабильные >1 года, ETL rerun завершён. **Pitfall 33 (resolved iter 79):** useCategoryPage split — circular dependency broken через `config.filterStore` injection. Pitfall 22 + 28 + 29 актуальны.
