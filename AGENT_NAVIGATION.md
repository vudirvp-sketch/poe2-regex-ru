...
> **Entry document.** Read this first. Current state: **iter 125** — фикс in-game FP `(A|B|C) after .* bridge`. Пользователь сообщил: при выборе «И» имплисетов `Редкость предметов: +##%` + `Эффективность монстров: +##%` с min=25, генератор выдавал `"едкость.*\+(2[5-9]|[3-9][0-9]|\d{3,})" "ивность.*\+(2[5-9]|[3-9][0-9]|\d{3,})"`, который в игре подсвечивал +15% / +11% (FP). Root cause: PoE2 in-game игнорирует содержимое `(A|B|C)`, когда `()` стоит ПОСЛЕ `.*` bridge + literal prefix — матчит prefix broadly. Симулятор парсил `(A|B|C)` корректно → тесты пропускали кейс. Фикс: `distributeAlternation()` в `src/core/compiler.ts` конвертирует `prefix(A|B|C)suffix` → `prefixAsuffix|prefixBsuffix|prefixCsuffix` (Path D — top-level `|`, in-game verified) — применяется в 3 местах `compileInner` для reversed `RANGE`. Дополнительно: `src/ui/hooks/category-ast-utils.ts` расширил определение `anchorEnd` для reversed implicits с `...##%` template → каждый Path-D альтернатив anchored к `%` (FP-протекция от range notation). +25 новых тестов в `tests/core/iter125-alt-after-bridge.test.ts`. KI#9 (MULTI_RANGE slot N>0 — та же проблема, не фиксировано) — см. STATUS.md. iter 124: cleanup stale `DELETIONS-iter123.txt`. iter 122: SeoBlock atmosphere. iter 121: ре-фикс hero decorations (KI#7). KI#7/KI#8/KI#9 — awaiting user verification. Актуальный статус — в `STATUS.md`, история — в `worklog.md`, полный план сортировки — в `docs/AFFIX_ORDERING_PLAN.md`, полный UI-аудит — в `docs/UI_AUDIT.md`.
