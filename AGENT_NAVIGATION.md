...
> **Entry document.** Read this first. Current state: **iter 132** — UI Refactor Phase 1 implementation (foundation). Phase 1 готова: 5 новых полей `FilterState` (`collapsedGroups`, `expandedSubGroups`, `showSelectedOnly`, `pinnedIds`, `chipExpandState`) + 13 actions + URL-сериализация (5 compact keys c/es/so/pn/ce, omit-when-default) + backward-compat (old URLs → defaults, defensive parsing of malformed values) + 46 тестов в `tests/store/filter-store.test.ts` (NEW — 9 describe blocks: initial state, asymmetric default, 5 action families, round-trip, backward-compat, compact serialization, resetFilters, clearSelections scope, store isolation). UI код НЕ тронут — только `src/store/filter-store.ts` (318→515 строк) extended + 1 new test file. `resetFilters()` сбрасывает новые поля; `clearSelections()` НЕ трогает их (different scope — selections = transient, collapse/pinned = user prefs). vitest 1988→2034 (+46), tsc 0, eslint 0. `docs/UI_REFACTOR_PLAN.md` §12 Phase 1 → ✅ DONE, §13.6 → «Recommendation for iter 133» (Phase 2 next). Phases 2/2.5/3/5 теперь могут потреблять готовые поля из store — infrastructure готова. KI#9 — monitoring. KI#7/KI#8/KI#10-KI#13 закрыты. Актуальный статус — в `STATUS.md`, история — в `worklog.md`, полный план UI-рефакторинга — в `docs/UI_REFACTOR_PLAN.md` (§12 Phase Status — Phase 1 ✅ DONE iter 132, остальные NOT STARTED), эталон визуализации — в `docs/UI_VISUALIZATION_AUDIT.md` (+§8 iter 131 corrections), полный план сортировки — в `docs/AFFIX_ORDERING_PLAN.md`, полный UI-аудит — в `docs/UI_AUDIT.md` (§10 TopNav dropdowns SUPERSEDED — visualization keeps flat nav).
