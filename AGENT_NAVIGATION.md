...
> **Entry document.** Read this first. Current state: **iter 101** (P0-фикс Critical Bug: `GameTokenSchema` не содержал `functionalCategory` → Zod strips → runtime classifier падал в `other` → ВСЕ affixes отображались как «Прочее» в production с iter 90; фикс = 1 строка в `src/shared/schemas.ts` + 3 регрессионных теста; 1414/1414 тестов зелёные, ESLint 0 errors + 2 library-level warnings, ETL 11 fresh / 0 stale, без изменений в `public/generated/*.json`). Regex-движок: чистый TS, 0 npm-зависимостей. ETL: 1697 токенов, FN=0, FP=9463. Актуальный статус — в `STATUS.md`, история — в `worklog.md`.
...
- **Опционально:** расширить e2e-тестирование — `tests/integration/` с тестами «load JSON → classify → verify не-`other` блоки» для всех 4 категорий (чтобы подобный regression не повторился).
