...
describe('compile', () => {
  ...
  it('compiles RANGE with prefix and suffix', () => {
    ...
    // range(60, undefined, 'больше опыта', 'даруют на') → "даруют на ([6-9].|[0-9]..).*больше опыта"
  ...
  it('compiles RANGE with prefix and both min+max', () => {
    ...
    // Note: without round10, ≥25 → (2[5-9]|[3-9].|[0-9]..), ≤30 → ([0-9]|[1-2].|30)
  ...
  it('compiles RANGE with exact=true (no round10)', () => {
    ...
    // 25 without round10 → (2[5-9]|[3-9].|[0-9]..) instead of ([2-9].|[0-9]..)
