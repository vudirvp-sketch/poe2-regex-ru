...
| Тестирование | Vitest 4 (2163 тестов), проверено в реальной игре (`docs/IN_GAME_TESTS.md`) |
...
tests/             # 49 test files (core/, shared/, etl/, ui/, integration/, store/)
...
# Тесты
pnpm test              # 2163/2163 должно пройти
...
| Файл | Назначение |
|------|-----------|
| [`AGENT_NAVIGATION.md`](./AGENT_NAVIGATION.md) | **Entry document для AI-агентов** — где что лежит, path aliases, dependency rules, 48 pitfall-паттернов |
| [`STATUS.md`](./STATUS.md) | Текущий статус итерации (iter 138), Known Issues, подтверждённые ограничения PoE2 |
...
| [`docs/UI_REFACTOR_PLAN.md`](./docs/UI_REFACTOR_PLAN.md) | План UI-рефакторинга на 7 фаз (iter 129 + review iter 130 + user feedback iter 131 + **Phase 1 DONE iter 132** + **Phase 2 DONE iter 133** + **Phase 2.5 DONE iter 134** + **Phase 3 DONE iter 135** + **Phase 5 DONE iter 136** + **Phase 4 + 4.5 DONE iter 137** + **iter 138: `--strong` modifier wiring** (optional enhancement): collapsible affix groups + sticky search + «+N ещё» chip expander + show-selected-only + SelectedBasket panel + favorites in left panel + ⭐ pin slot on FilterChip + stronger bg tints + compact chip density 25% + portal-based tooltips + «Обозначения» icon legend + `--strong` modifier в tier-first mode). **ВСЕ 7 ФАЗ UI REFACTOR DONE + iter 138 ENHANCEMENT.** |
