...
| Тестирование | Vitest 4 (2079 тестов), проверено в реальной игре (`docs/IN_GAME_TESTS.md`) |
...
tests/             # 45 test files (core/, shared/, etl/, ui/, integration/, store/)
...
# Тесты
pnpm test              # 2079/2079 должно пройти
...
| Файл | Назначение |
|------|-----------|
| [`AGENT_NAVIGATION.md`](./AGENT_NAVIGATION.md) | **Entry document для AI-агентов** — где что лежит, path aliases, dependency rules, 46 pitfall-паттернов |
| [`STATUS.md`](./STATUS.md) | Текущий статус итерации (iter 136), Known Issues, подтверждённые ограничения PoE2 |
...
| [`docs/UI_REFACTOR_PLAN.md`](./docs/UI_REFACTOR_PLAN.md) | План UI-рефакторинга на 7 фаз (iter 129 + review iter 130 + user feedback iter 131 + **Phase 1 DONE iter 132** + **Phase 2 DONE iter 133** + **Phase 2.5 DONE iter 134** + **Phase 3 DONE iter 135** + **Phase 5 DONE iter 136**: collapsible affix groups + sticky search + «+N ещё» chip expander + show-selected-only + SelectedBasket panel + favorites in left panel + ⭐ pin slot on FilterChip). Phase 1+2+2.5+3+5 готовы — 5 полей `FilterState` + 13 actions + URL sync + GroupHeader + sticky search + expand/collapse-all кнопки + per-sub-group chip truncation + show-selected-only filter + SelectedBasket с cap=20 + collapsible right aside + LeftPanelFavorites с click-to-scroll + ⭐ pin/unpin icon. |
