 ...
 *
 *  iter 129 cleanup: removed 'больше.*волшебн.*редк.*монстр' (Bug #2 fix iter 84).
 *  That BTS pattern ("На #% больше волшебных и редких монстров") is now filtered
 *  at ETL via `WAYSTONE_IMPLICIT_SET_FAMILY_KEYS` (iter 128 KI#13) and never
 *  reaches the classifier — pattern was dead.
 *
...
const POSITIVE_KEYWORDS = /(?:повышен.*редкост|повышен.*количеств|увеличен.*редкост|увеличен.*количеств|повышен.*опыт|увеличен.*опыт|качество|дополнит.*сгустк|шанс.*сгустк|дополнит.*путев|дополнит.*сундук|больше.*опыт|больше.*золот|больше.*путев|больше.*предмет|дополнит.*дух|дополнит.*Бездн|дополнит.*бездн|дополнит.*ларец|дополнит.*Сущност|дополнит.*изгнан|приспешник.*дополнит|Бездны ведут|дополнит.*ритуальн|дополнит.*алтар|Сложность монстров Бездны.*наград|дополнит.*Царевн|Бездн появляется.*редких)/i;

/** Keywords indicating a negative (detrimental) waystone mod — makes the map harder.
 *  Bug #2 fix (iter 84): 'бонус.*крит.*урон.*монстр' — "+##% к бонусу критического
 *  урона монстров" (1 group) — REAL mod, still in data, kept.
 *
 *  iter 129 cleanup: removed 2 BTS patterns (Bug #2 iter 84 additions):
 *  - 'шанса появления свойств.*редк.*монстр' — BTS, filtered at ETL (iter 128 KI#13).
 *  - 'больше.*эффективн.*монстр' — BTS, filtered at ETL (iter 128 KI#13).
 *  Both were dead (BTS tokens removed from data, never reach classifier).
 *
 ...
 *
 *  iter 129 cleanup: removed 3 BTS-only patterns (dead after iter 128 KI#13):
 *  'количеств.*редк.*монстр', 'количеств.*волшебн.*монстр', 'больше.*волшебн.*редк.*монстр'
 *  — all match BTS tokens filtered at ETL. */
const POSITIVE_LOOT_PATTERNS = /(?:редкост|количеств.*предмет|количеств.*путев|количеств.*золот|больше.*золот|больше.*путев|больше.*предмет|дополнит.*сундук|дополнит.*ларец|дополнит.*изгнан|дополнит.*Сущност|путев)/i;
 ...
 *
 *  iter 129 cleanup: removed 'шанса появления свойств.*редк.*монстр' (BTS, filtered
 *  at ETL iter 128 KI#13). Kept 'Дополнительных свойств у редких монстр' — REAL mod. */
const NEGATIVE_MONSTER_MODIFIERS_PATTERNS = /(?:Дополнительных свойств у редких монстр)/i;
...
/** A-tier waystone prefixes: Experience, Chests.
 *  iter 129 cleanup: removed 'волшебн.*монстр|редк.*монстр' — BTS-only patterns
 *  (filtered at ETL iter 128 KI#13). Real A-tier affixes: experience + chests. */
const WAYSTONE_A_PREFIX = /(?:опыт|волшебн.*сундук|редк.*сундук)/i;
