...
describe('computeExcludePatterns: short marker priorities', () => {
  ...
  it('combines minion + compound markers for mixed conflict types', () => {
    // "повышение шанса критического удара" has FP from:
    // - minion: "Приспешники имеют X% повышение шанса критического удара"
    // - compound: "повышение шанса критического удара для"
    // Should combine "Приспеш" + " для" (or similar) instead of long specific patterns
    const tokens: NormalizedMod[] = [
      makeMod({
        id: 'test.crit_player',
        rawText: { ru: '+(20—30)% повышение шанса критического удара' },
        rawTextTemplate: { ru: '+##% повышение шанса критического удара' },
      }),
      makeMod({
        id: 'test.crit_minion',
        rawText: { ru: 'Приспешники имеют (20—30)% повышение шанса критического удара' },
        rawTextTemplate: { ru: 'Приспешники имеют (##)% повышение шанса критического удара' },
      }),
      makeMod({
        id: 'test.crit_for',
        rawText: { ru: '+(20—30)% повышение шанса критического удара для' },
        rawTextTemplate: { ru: '+##% повышение шанса критического удара для' },
      }),
    ];
    ...
    // Should have excludes (not a clean unique suffix since FP exists)
    if (result.regexExclude && result.regexExclude.length > 0) {
      // At least one exclude should be a short marker (≤10 chars)
      const hasShortExclude = result.regexExclude.some(exc => exc.length <= 10);
      expect(hasShortExclude).toBe(true);

      // Total exclude length should be shorter than old long specific patterns
      // Old: "повышение шанса критического удара для" = 39 chars
      const excludeTotal = result.regexExclude.reduce((sum, exc) => sum + exc.length, 0);
      expect(excludeTotal).toBeLessThan(40);
    }
  });

  it('uses short first-word exclude instead of long full-phrase', () => {
    // "скорости сотворения чар" has FP from flask-effect variants:
    // "...скорости сотворения чар во время действия любого флакона"
    // Should use "флакона" or "во время" as short exclude
    const tokens: NormalizedMod[] = [
      makeMod({
        id: 'test.cast_speed',
        rawText: { ru: '+(20—30)% увеличение скорости сотворения чар' },
        rawTextTemplate: { ru: '+##% увеличение скорости сотворения чар' },
      }),
      makeMod({
        id: 'test.cast_speed_flask',
        rawText: { ru: '(20—30)% увеличение скорости сотворения чар во время действия любого флакона' },
        rawTextTemplate: { ru: '(##)% увеличение скорости сотворения чар во время действия любого флакона' },
      }),
    ];
    ...
    if (result.regexExclude && result.regexExclude.length > 0) {
      // Should prefer short markers over long specific patterns
      const maxExcludeLen = Math.max(...result.regexExclude.map(e => e.length));
