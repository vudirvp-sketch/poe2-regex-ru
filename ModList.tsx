...
interface ModListProps {
  ...
  /** When true, within each origin section further sub-group by jewel type
   *  (Рубин/Изумруд/Сапфир/Общие) instead of just flat chip list.
   *  Only meaningful when groupMode='origin'. */
...
export const ModList: React.FC<ModListProps> = ({
  ...
  const isJewelTypeMode = groupMode === 'jewel-type';
  ...
  return (
    <div className="mod-list flex flex-col gap-3" role="group" aria-label={t('search.placeholder')}>
      ...
      {familyGroups.length > 0 ? (
        isOriginMode || isJewelTypeMode ? (
          /* Origin/JewelType mode: single column, sub-grouped by origin (and optionally jewel type within) */
          <div className="flex flex-col gap-2">
            {classifyGroups(familyGroups, isJewelTypeMode ? 'jewel-type' : 'origin').map((sg) => (
              <div key={sg.key}>
                <div className={`text-xs font-bold uppercase tracking-wider mb-1.5 ${sg.colorClass}`}>
                  {sg.label} ({sg.groups.length})
                </div>
                {/* Within each section, further split by affix */}
                {(() => {
                  const sectionPrefix = sg.groups.filter(g => g.affix === 'prefix');
                  const sectionSuffix = sg.groups.filter(g => g.affix === 'suffix');
                  return (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 ml-2">
                      {sectionPrefix.length > 0 && (
                        <div className="border-l-2 border-blue-800/50 pl-3">
                          <h5 className="text-[10px] font-semibold text-blue-400 uppercase mb-1">{t('affix.prefix')} ({sectionPrefix.length})</h5>
                          {showJewelTypeSubGroups ? (
                            /* Sub-group by jewel type within affix column */
                            classifyGroups(sectionPrefix, 'jewel-type').map((jtSg) => (
                              <ModSubGroupSection
                                key={jtSg.key}
                                subGroup={jtSg}
                                selectedIds={selectedIds}
                      ...
                      {sectionSuffix.length > 0 && (
                        <div className="border-l-2 border-orange-800/50 pl-3">
                          <h5 className="text-[10px] font-semibold text-orange-400 uppercase mb-1">{t('affix.suffix')} ({sectionSuffix.length})</h5>
                          {showJewelTypeSubGroups ? (
                            /* Sub-group by jewel type within affix column */
                            classifyGroups(sectionSuffix, 'jewel-type').map((jtSg) => (
                              <ModSubGroupSection
                                key={jtSg.key}
                                subGroup={jtSg}
                                selectedIds={selectedIds}
